...
class NetworkEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        # print(f"{self.elephant_flow}")
        traffic_rate = 3000
        if self.elephant_flow in self.flows_traffic.keys():
            old_traffic_rate = self.flows_traffic[self.elephant_flow]
            self.total_incoming_network -= old_traffic_rate
            
        self.total_incoming_network += traffic_rate
        self.flows_traffic[self.elephant_flow] = traffic_rate
    ...
    def step(self, action):
        ...
        # print(self.flows_paths[self.elephant_flow])
