...
from new_model.new_model_utils import json2networkx
import random
...
class NetworkModel:
    ...
    def calculate_measurements(self, flows_traffic, flows_paths):
        ...
        for flow_name, traffic in self.flows_traffic.items():
            path = self.ziped_flow_paths[flow_name]
        ...
        for i in range(18):
            ...
            for switch, attributes in self.switches:
                ...
                # if switch == "4_3":
                #     print(f"iteration {i}: total incoming switch {switch}: {total_incoming}")
                ...
                # if switch == "4_3":
                #     print(f"Iteration {i}: switch {switch} leaving prop: {leaving_probability}")
            ...
            for flow, zipped_path in self.ziped_flow_paths.items():
                ...
                for (u, v) in zipped_path:
                    ...
                    if v not in [s[0] for s in self.switches]:
                        continue
        ...
        for switch, attributes in self.switches:
                ...
                total_incoming = self.full_AVTM_matrix[:, v_idx, :].sum()
                # if switch == "4_3":
                #     print(f"total incoming for switch {switch}: {total_incoming}")
                ...
                # if switch == "4_3":
                #     print(f"total outgoing for switch {switch}: {total_outgoing}")
                ...
                print(f"exp_delay_at switch {switch}: {exp_delay_at_switch}")
        ...
        for i, ((src, dst), traffic) in enumerate(self.flows_traffic.items()):
            ...
            for node in path[1:-1]:
                if node in self.switches:
                    total_delay += delay_at_switch[node]
...
if __name__ == '__main__':
    ...
    for flow in range(no_of_flows):
        edge_switches = (0, 1, 2, 3, 4, 5, 10, 15, 20, 21, 22, 23, 24, 19, 14, 9)
        random_hosts = random.sample(edge_switches, 2)
        random_traffic_rate = random.uniform(10, 300)
        total_incoming_network += random_traffic_rate
        if (f"10.0.1.{random_hosts[0]}",f"10.0.1.{random_hosts[1]}") not in flows_traffic:
            flows_traffic[(f"10.0.1.{random_hosts[0]}",f"10.0.1.{random_hosts[1]}")] = 0
